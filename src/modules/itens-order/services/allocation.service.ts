...
@Injectable()
export class AllocationService implements ItemInterface {

  constructor(
    @InjectRepository(AllocationEntity)
    private repository: Repository<AllocationEntity>,
  ...
  private createFromDto(createDto: CreateItemDto): AllocationEntity {
  ...
  private async addIntoOrderByDto(dto: AddItemIntoOrderDto): Promise<ItemOrderEntity> {
    ...
    entity.type = Constants.TYPE_ITEM.PRODUCT
